    ...
    <div className="min-h-screen bg-yellow-50">
      <nav className="bg-rose-800 text-white px-6 py-4 shadow-md flex justify-between items-center">
        ...
        <div className="space-x-10">
          <Link href="/resource-manager/dashboard" className="hover:underline">Dashboard</Link>
          <Link href="/resource-manager/crew-availability" className="hover:underline">Crew Availability</Link>
          <Link href="/resource-manager/Aircraft-maintenance" className="hover:underline">Aircraft Maintenance</Link>
          <Link href="/resource-manager/Assign-Gate-Runway" className="underline font-semibold">Assign Gate 🛫</Link>
        </div>
      </nav>
      ...
      <div className="p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dummyFlights.map((flight) => {
            ...
            return (
              <div key={flight.id} className={`bg-white p-5 rounded-2xl shadow-md border hover:shadow-xl transition ${isApproved ? "opacity-70" : ""}`}>
                ...
                <div className="mt-4 space-y-2">
                  <div>
                    ...
                    <select disabled={isApproved} onChange={(e) => handleChange(flight.id, "gate", e.target.value)} className="border rounded px-2 py-1">
                      <option value="">Select</option>
                      {availableGates.map((gate) => <option key={gate}>{gate}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="mr-2 font-medium">Runway:</label>
                    <select disabled={isApproved} onChange={(e) => handleChange(flight.id, "runway", e.target.value)} className="border rounded px-2 py-1">
                      <option value="">Select</option>
                      {availableRunways.map((runway) => <option key={runway}>{runway}</option>)}
                    </select>
                  </div>

                  <h4 className="mt-3 font-semibold text-rose-700">👨‍✈️ Crew Assignment</h4>
                  <input disabled={isApproved} type="text" placeholder="Pilot" onChange={(e) => handleCrewChange(flight.id, "pilot", e.target.value)} className="w-full border px-2 py-1 rounded" />
                  <input disabled={isApproved} type="text" placeholder="Copilot" onChange={(e) => handleCrewChange(flight.id, "copilot", e.target.value)} className="w-full border px-2 py-1 rounded" />
                  {[0, 1].map((i) => (
                    <input disabled={isApproved} key={i} type="text" placeholder={`Attendant ${i + 1}`} onChange={(e) => handleCrewChange(flight.id, "attendants", e.target.value, i)} className="w-full border px-2 py-1 rounded" />
                  ))}
                  {[0, 1].map((i) => (
                    <input disabled={isApproved} key={i} type="text" placeholder={`Ground Staff ${i + 1}`} onChange={(e) => handleCrewChange(flight.id, "groundStaff", e.target.value, i)} className="w-full border px-2 py-1 rounded" />
                  ))}

                  <button
                    disabled={isApproved}
                    onClick={() => handleSubmit(flight.id)}
                    className={`mt-4 ${isApproved ? "bg-gray-400 cursor-not-allowed" : "bg-green-700 hover:bg-green-800"} text-white px-4 py-2 rounded w-full`}
                  >
